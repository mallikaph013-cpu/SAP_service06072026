...
namespace myapp.Controllers
{
    ...
    public class RequestsController : Controller
    {
        ...
        {
            await EnsureDocumentNumbersAsync();
        ...
        public async Task<IActionResult> Create(CreateRequestViewModel viewModel, IFormFile? requestAttachment)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (viewModel != null)
                {
                    var sapModuleCode = NormalizeSapModuleCode(viewModel.SapModule);
                    var isRequestType = viewModel.RequestType == RequestType.Request;
                    var yearPart = requestDateUtc.ToString("yy", CultureInfo.InvariantCulture);
                    var middleSegment = isRequestType ? "SR-" : string.Empty;
                    var prefix = string.IsNullOrWhiteSpace(sapModuleCode)
                        ? $"{middleSegment}{yearPart}-"
                        : $"{sapModuleCode}-{middleSegment}{yearPart}-";
                    ...
                    {
                        var sequencePart = existingNumber.Split('-', StringSplitOptions.RemoveEmptyEntries).LastOrDefault();
                        if (int.TryParse(sequencePart, out var parsedSequence) && parsedSequence > maxSequence)
                        {
                ...
                var requestItem = new RequestItem
                {
                    ...
                    ChangeRequestG2U = viewModel?.ChangeRequestG2U ?? String.Empty,
                    ...
                    QuotationNumber = IsChangeRequestModule(viewModel?.SapModule)
                        ? viewModel?.ChangeRequestNoG2U
                        : viewModel?.QuotationNumber,
        ...
        //user ที่ login เข้ามา
            ...
            var viewModel = new CreateRequestViewModel
            {
                ...
                ChangeRequestNoG2U = IsChangeRequestModule(requestItem.SapModule)
                    ? requestItem.QuotationNumber
                    : null,
            ...
            }
        {
           ...
           // if (ModelState.IsValid)
           {
                ...
                try
                {
                    ...
                    requestItemToUpdate.QuotationNumber = IsChangeRequestModule(viewModel.SapModule)
                        ? viewModel.ChangeRequestNoG2U
                        : viewModel.QuotationNumber;
        ...
        private async Task EnsureDocumentNumbersAsync()
        {
            var requestsToFix = await _context.RequestItems
                .Where(r => r.UsageStatus != 9
                    && (string.IsNullOrWhiteSpace(r.DocumentNumber)
                        || (r.DocumentNumber != null && r.DocumentNumber.ToUpper().Contains(" MODULE-"))))
                .OrderBy(r => r.RequestDate)
                .ThenBy(r => r.Id)
                .ToListAsync();

            if (!requestsToFix.Any())
            {
                return;
            }
            ...
            var maxSequenceByPrefix = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase);

            int GetMaxSequenceForPrefix(string prefix)
            {
                if (maxSequenceByPrefix.TryGetValue(prefix, out var knownMax))
                {
                    return knownMax;
                }
                ...
                foreach (var number in existingNumbers)
                {
                    ...
                    var sequencePart = number.Split('-', StringSplitOptions.RemoveEmptyEntries).LastOrDefault();
                    if (int.TryParse(sequencePart, out var parsedSequence) && parsedSequence > max)
                    {
                        max = parsedSequence;
                    }
                }

                maxSequenceByPrefix[prefix] = max;
                return max;
            }

            foreach (var request in requestsToFix)
            {
                if (!string.IsNullOrWhiteSpace(request.DocumentNumber))
                {
                    request.DocumentNumber = NormalizeLegacyDocumentNumber(request.DocumentNumber);
                    continue;
                }

                var sapModuleCode = NormalizeSapModuleCode(request.SapModule);
                var isRequestType = string.Equals(request.RequestType, RequestType.Request.ToString(), StringComparison.OrdinalIgnoreCase);
                var yearPart = request.RequestDate.ToString("yy", CultureInfo.InvariantCulture);
                var middleSegment = isRequestType ? "SR-" : string.Empty;
                var prefix = string.IsNullOrWhiteSpace(sapModuleCode)
                    ? $"{middleSegment}{yearPart}-"
                    : $"{sapModuleCode}-{middleSegment}{yearPart}-";

                var nextSequence = GetMaxSequenceForPrefix(prefix) + 1;
                request.DocumentNumber = $"{prefix}{nextSequence:000}";
                maxSequenceByPrefix[prefix] = nextSequence;
            }

            await _context.SaveChangesAsync();
        }

        private static string NormalizeSapModuleCode(string? sapModule)
        {
            ...
            var normalized = sapModule.Trim().ToUpperInvariant();

            if (normalized.EndsWith(" MODULE", StringComparison.Ordinal))
            {
                normalized = normalized[..^7].Trim();
            }
            else if (normalized.EndsWith("MODULE", StringComparison.Ordinal))
            {
        ...
        private static string NormalizeLegacyDocumentNumber(string documentNumber)
        {
            const string marker = " MODULE-";
            var markerIndex = documentNumber.IndexOf(marker, StringComparison.OrdinalIgnoreCase);
            if (markerIndex < 0)
            {
                return documentNumber;
            }

            return documentNumber.Remove(markerIndex, marker.Length - 1);
        }

        private static bool IsChangeRequestModule(string? sapModule)
        {
            return string.Equals(NormalizeSapModuleCode(sapModule), "CR", StringComparison.OrdinalIgnoreCase);
        }
            ...
            switch (requestType)
            {
                case RequestType.Request:
                    headers.AddRange(new[]
                       {
                        "ChangeRequestG2U", "Description", "AttachmentFileName", "AttachmentPath"
                 });
                    break;
